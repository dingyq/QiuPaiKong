...
@implementation CircleInfoDB

+ (instancetype)getInstance {
    static CircleInfoDB *shareInstance = nil;
...
- (instancetype)init {
    ...
    _tableName = [[configDic objectForKey:@"TableName"] objectAtIndex:0];
...
- (NSMutableArray *)getLatestDataFromDB:(NSInteger)maxSortId {
    ...
    if (maxSortId == 0) {
        ...
        sql = [NSString stringWithFormat:@"select * from %@ where sortId < %ld order by sortId desc LIMIT %ld", _tableName, (long)maxSortId, (long)kPageSizeCount];
    }
